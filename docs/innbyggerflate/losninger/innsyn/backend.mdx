...
Dette er hovedsakelig backend-løsninger for [**`dp-dagpenger`**][dp-dagpenger] . I tillegg tilbyr denne applikasjonen spesifikke endepunkter for både [**`dp-soknad`**][dp-soknad] og [**`aia-backend`**][aia-backend] eid av paw-teamet.

## Formål

[**`dp-innsyn`**][dp-innsyn] er utviklet for å forenkle prosessen med å hente informasjon relatert til søknader og vedtak. Kjernefunksjonene omfatter å hente innsendte og pågående søknader, hente vedtak samt innhente behandlingsstatus for søknader. I tillegg håndterer applikasjonen hendelser relatert til søknader, vedtak og journaler ved hjelp av hendelsesmottak.
